...
#Input
-matrix_file(args[1])		:Expression data set
-list_file(args[2])			:Table containing target gene / protein name
-list_column(args[3])		:Column name containing target gene / protein name
-annotation_table(args[4])  :Corresponding table between the gene / protein names and uniprot accession IDs
-n_cluster(arg[5])		    :No of cluster for k-mean clustering
...
def multi_corr2corr(expr_matrix_dir, corr_list_dir, columns_name):
    '''
    example
    -corr_list_dir="4c.spermatid_specific_for_GO.csv"
    -columns_name="M.S.Symbol"
    -expr_matrix_dir = "Fig.4B_from_scanpy_filtered_(2-1a.expression_matrix).csv"
    '''
...
def matrix2go(corr2corr_list, annotation_file, obo_file, methods):
  #annotation_file = ./annotation_(scanpy_var).csv
  #obo_file = ./go-basic.obo
  #methods = ["bonferroni", "sidak", "holm", "fdr", "fdr_bh"]
  os.mkdir("./GO_result/")
  go = obo_parser.GODag(obo_file)
  with gzip.open("./input/*.gaf.gz", 'rt') as mouse_gaf_fp:
...
def main_clustering2plot(n_clusters, run_elbow, dendrogram, format_type, show):
    def elbow(jc_matrix):
        sum_of_squared_errors = []
        for i in range(1,30):
            Kmodel = KMeans(n_clusters=i, random_state=0, init='random')
            Kmodel.fit(jc_matrix)
            sum_of_squared_errors.append(Kmodel.inertia_)
...
if __name__ == "__main__":
	...
	main_clustering2plot(n_clusters=n_clusters, run_elbow=False, dendrogram=True, format_type='eps', show=True)
	kmeans2jc_main(n_clusters=n_clusters, format_type='eps', dendrogram=True, show=True)
	"""GO and gene table"""
